...
export function RaahiBrandLockup({ className = "", compact = false, inverse = false }: RaahiBrandLockupProps) {
  return (
    <span className={`inline-flex items-center gap-3 ${className}`} aria-label="Raahi">
      ...
      <span className={`${compact ? "text-base" : "text-xl"} font-black uppercase leading-none tracking-[-0.06em] ${inverse ? "text-white" : "text-[#17251b]"}`}>
        RAA<span className={inverse ? "text-raahi-200" : "text-[#32b45c]"}>HI</span>
      </span>
    </span>
  );
}
