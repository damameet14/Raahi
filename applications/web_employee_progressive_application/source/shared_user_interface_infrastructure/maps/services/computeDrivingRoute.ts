...
export async function computeDrivingRoute({
  ...
  validateDrivingRouteCalculationRequest({ apiKey, destination, origin, signal });
...
function toRouteWaypoint(coordinates: Coordinates) {
  return {
    location: { latLng: { latitude: coordinates.lat, longitude: coordinates.lng } },
  };
}
...
function decodeEncodedPolyline(encodedPolyline: string): Coordinates[] {
  ...
  while (index < encodedPolyline.length) {
    ...
    path.push({ lat: lat / 100_000, lng: lng / 100_000 });
  }
...
function decodeNextValue(
  ...
  do {
    if (index >= encodedPolyline.length) {
      throw new RouteNotFoundError("Routes API returned an invalid route path.");
    }
